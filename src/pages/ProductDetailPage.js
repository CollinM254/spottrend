    ...
    const buyNowHandler = async (productId, product) => {
        try {
            ...
            if (response.status === 201) {
                ...
                // Assuming the seller's phone number is stored in an environment variable or directly in the code for now
                const sellerPhoneNumber = '254741854304'; // Update with the actual seller phone number
                const message = `Hello Trendspot, I want the ${product.name} that you are selling at Ksh${product.price}. Is it available?`;
                const whatsappUrl = `https://wa.me/${sellerPhoneNumber}?text=${encodeURIComponent(message)}`;
                window.open(whatsappUrl, '_blank');
            } else {
                console.error('Failed to create order:', response.data);
                ...
                {productDetails ? (
                    <div className="flex flex-col lg:flex-row">
                        ...
                        <div className="w-11/12 lg:w-1/2 pl-8 flex flex-col justify-center">
                            ...
                            <p className="text-2xl font-bold text-orange-500">Ksh{productDetails.price}</p>
