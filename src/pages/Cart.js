...
const Cart = () => {
    ...
    const generateWhatsAppUrl = (cartItems, total) => {
        let message = "Hello Trendspot, I want the following products:\n";
        cartItems.forEach(item => {
            message += `${item.product.name}: Quantity ${item.quantity}, Price Ksh ${item.price}\n`;
        });
        message += `\nTotal Price: Ksh ${total.toLocaleString('en-IN', { maximumFractionDigits: 2 })}\nAre they available?`;
    
        // Encode the message to be used in a URL
        message = encodeURIComponent(message);
    
        // Replace `YOUR_SELLER_PHONE_NUMBER` with the actual number
        const sellerPhoneNumber = '254741854304';
        return `https://wa.me/${sellerPhoneNumber}?text=${message}`;
    };
    ...
    const placeOrder = async () => {
        try {
            ...
            if (response.status === 201) {
                ...
                const whatsappUrl = generateWhatsAppUrl(cartItems, totalAmount);
                ...
                {cartItems.length === 0 ? (
                    ...
                    <div className='w-full sm:max-w-[70%] m-auto'>
                        ...
                        <div className="w-[90%] m-auto sm:w-full my-4 flex flex-row-reverse">
                            ...
                            >
                                Checkout
                                <p className="font-bold inline">: Ksh {calculateTotal().toLocaleString('en-IN', { maximumFractionDigits: 2 })}</p>
                            </button>
                            <button
                                onClick={clearCart}
